...
# # observed_with_pair_id1 = pd.merge(observed, pair_df, left_on="source_id",
...
# # observed_with_pair_id2 = pd.merge(observed, pair_df, left_on="source_id",
# #                                    right_on="source_id2", how="inner")
...
print(len(np.unique(np.concatenate((star1, star2)))), "stars observed")
...
def find_new_stars(star, star_array):
    ...
    if len(star_array[:, 0][m1]):
        print(star_array[:, 0][m1], star_array[:, 1][m1])
        print(star_array[:, 1][m2], star_array[:, 0][m2])
...
def find_group(i, star_array):
    print(tups[i][0], tups[i][0], "\n")
    find_new_stars(tups[i][0], star_array)

# for i in range(1):
#     find_group(i, unique_list[inds])
